...
Tasks.add = function(input, callback) {
	...
	if (!input.list) {
		errorMessages.push('Object field "list" is required.');
	}
	...
	if (errorMessages.length > 0) {
		...
		async.parallel({
			...
			list: function(cb) {
				internals.data.List.find(parseInt(input.list)).success(function(list) {
					cb(null, list);
				});
			}
		},
			...
			if (errorMessages.length > 0) {
				...
				internals.data.Task.create({ name: input.name, status: task_status }).success(function(newTask) {
					newTask.setUser(user).success(function() {
						newTask.setList(list).success(function() {
							//success
...
Tasks.getAll = function(input, callback) {
	...
	Faire.Lists.isListVisible({user: input.user, id: input.list}, function(err, listIsVisible) {
		...
		if (errorMessages.length > 0) {
			...
			if (listIsVisible) {
				internals.data.Task.findAll({ where: { ListId: input.list }, include: [internals.data.User] }).success(function(tasks) {
					var output = [];
...
Tasks.getListOfTask = function(input, callback) {
	...
	if (errorMessages.length > 0) {
		...
		internals.data.Task.find({ where: { id: input.id } }).success(function(task) {
			var output = null;
