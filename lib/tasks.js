...
Tasks.activate = function(input, callback) {
    ...
    if (errorMessages.length > 0) {
        ...
        internals.data.task.find({ where: { id: input.id, userId: input.user }, include: [internals.data.user] }).then(function(task) {
...
Tasks.getAll = function(input, callback) {
    ...
    Faire.Lists.isListVisible({user: input.user, id: input.list}, function(err, listIsVisible) {
        ...
        if (errorMessages.length > 0) {
            ...
            if (listIsVisible) {
                internals.data.task.findAll({ where: { listId: input.list }, include: [internals.data.user] }).then(function(tasks) {
                    var output = [];
