    ...
    (function(model) {
        ...
        scurvy.setupSync(model, function(err) {
			...
			async.parallel({
				...
				list: function(cb) {
					model.List.sync(syncConfig).success(function() {
						cb();
					}).error(function(error) { console.log('Error during List.sync(): ' + error); });
				},
				sharedLists: function(cb) {
					model.SharedLists.sync(syncConfig).success(function() {
						cb();
					}).error(function(error) { console.log('Error during SharedLists.sync(): ' + error); });
				}
