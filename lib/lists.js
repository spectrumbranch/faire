...
internals.getAllClosure = function(list, userid) {
    return function(done) {
        ...
        // output_single.sharedUsers = list.sharedUsers ? list.sharedUsers : [];
...
Lists.activate = function(input, callback) {
    ...
    if (errorMessages.length > 0) {
        ...
        internals.data.list.find({ where: { id: input.id, OwnerId: input.user }, include: [{ model: internals.data.user, as: 'Owner' }] }).then(function(list) {
...
Lists.delete = function(input, callback) {
	console.log('inside list.delete');
    var errorMessages = [];
    //test existence of fields
    if (!input.id) {
	...
	console.log('inside list.delete after validation');
    if (errorMessages.length > 0) {
        callback(new Error(errorMessages));
    } else {
		
		console.log('inside list.delete passed validation');
        internals.data.list.find({ where: { id: input.id, OwnerId: input.user }, include: [{ model: internals.data.user, as: 'Owner' }] }).then(function(list) {
            
			console.log('inside list.delete after find', list);
			list.updateAttributes({
                status: 'deleted'
            }).then(function(output_list) {
				
				console.log('inside list.delete after updates');
                var output = {};
