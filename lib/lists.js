...
Lists.getAll = function(input, callback) {
	...
	if (errorMessages.length > 0) {
		...
		internals.data.List.findAll({ 
			...
			var async = require('async');
			async.parallel(internals.getAllParallelClosure(lists, input.user), function(err, results) {
...
Lists.share = function(input, callback) {
	...
	if (!input.users || !Array.isArray(input.users) || (Array.isArray(input.users) && input.users.length == 0)) {
		errorMessages.push('Object field "users" is required and must be a non-empty array.');
	}
	if (!input.id) {
		errorMessages.push('Object field "id" is required.');
	}
	...
	if (errorMessages.length > 0) {
		...
		internals.data.List.find(input.id).success(function(list) {
			var async = require('async');
			async.parallel(internals.shareParallelClosure(list,input.users), function(err, results) {
				callback(err, results);
			})
		});
	}
...
Lists.unshare = function(input, callback) {
	...
	if (errorMessages.length > 0) {
		...
		internals.data.List.find(input.id).success(function(list) {
			var async = require('async');
			async.parallel(internals.unshareParallelClosure(list,input.users), function(err, results) {
				callback(err, results);
			})
		});
	}
