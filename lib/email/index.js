...
internals.sendEmail = function(cb) {
	return internals.checkAuthentication(function(err, auth) {
		if (err) {
			console.error('Google OAuth2 Authentication failed, an admin may need to setup credentials.', err);
			return cb(err);
		} else {
			return internals.sendMessage(auth, cb);
		}
	});
}
	...
	return internals.checkAuthentication(function(err, auth) {
	  if (err) {
	    return console.log('err', err);
	  	...
	  	const raw = internals.makeBody(properties.to, properties.from, properties.subject, properties.body);
