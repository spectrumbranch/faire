module.exports.get = function(Faire) {
	...
	return [
				...
				handler: function() {
					...
					if (session) {
						if (taskname && taskname != '') {
							...
							request.reply({error: 'name is a required field.'});
						}
					} else {
						request.reply({error: 'Must be logged in.'});
					}
	  ...
	  //All static content
	  { method: '*', path: '/{path*}', config: { handler: { directory: { path: './static/', listing: false, redirectToSlash: true } }, auth: { mode: 'try', strategy: 'session' } } }
	];
};